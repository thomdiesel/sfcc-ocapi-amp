...
function calculateProductPrices (basket) {
    ...
    while (productQuantitiesIt.hasNext()) {
        ...
        var prodPrice = null;
        if (prod.ID === 'external-product') {
            var externalProduct = basket.getProductLineItems('external-product');
            collections.forEach(externalProduct, function(product) {
                prodPrice = product.basePrice;
            });
        } else {
            prodPrice = prod.priceModel.getPrice(quantity);
        }
        productPrices.put(prod, prodPrice);
    }
