...
class OrderHistoryModel extends BaseModel {
  fetch() {
    ...
    // TODO: This stuff should be handled by API client 
    const customerId = this.__config.customer_id;
    const bearerToken = this.__config.bearer_token;
    const orderHistoryUrl = `${process.env.OCAPI_HOST}${process.env.OCAPI_PATH}/customers/${customerId}/orders?client_id=${process.env.OCAPI_CLIENT_ID}`;
